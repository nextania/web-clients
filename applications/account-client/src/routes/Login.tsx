...
import { getBrowser } from "../utilities/client";
...
const Login = ({ loading, setLoading, escalate }: { loading: Accessor<boolean>; setLoading: Setter<boolean>; escalate?: boolean; }) => {
    ...
    const login = async () => {
        ...
        if (stage() === "credentials") {
            ...
            setEmail(match[0]);
            try {
                const session = await createSession(email().trim(), password(), escalate ? s()!.token : undefined, persist(), getBrowser());
                ...
                setError(RenderableError.fromError(e as RequestError).render());
                setLoading(false);
            }
        } else if (stage() === "2fa") {
            if (!code()) {
